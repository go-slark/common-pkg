...
func (q *LoggerForwardingQueue) run() {
	...
	for {
		...
		if err != nil {
			q.logger.Log(ctx, xlogger.ErrorLevel, map[string]interface{}{}, fmt.Sprintf("Impossible to marshal log payload to JSON: %v (payload: %v)", err, payload))
			continue
		}
		var payloadJSON map[string]interface{}
		err = json.Unmarshal(payloadBytes, &payloadJSON)
		if err != nil {
			q.logger.Log(ctx, xlogger.ErrorLevel, map[string]interface{}{}, fmt.Sprintf("Impossible to unmarshal log payload into map[string]interface{}: %v (payload: %s)", err, payloadBytes))
			continue
		}
		...
		if payload.Response.Status >= http.StatusInternalServerError {
			q.logger.Log(ctx, xlogger.ErrorLevel, payloadJSON, "server error")
		} else if payload.Response.Status >= http.StatusBadRequest {
			q.logger.Log(ctx, xlogger.WarnLevel, payloadJSON, "client error")
		} else {
			q.logger.Log(ctx, xlogger.InfoLevel, payloadJSON, "request processed")
		}
	}
}
