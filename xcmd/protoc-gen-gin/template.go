...
func _{{$svrType}}_{{.Name}}{{.Num}}_HTTP_Handler(srv {{$svrType}}HTTPServer) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if err != nil {
			err = xerror.NewError(xerror.FormatInvalidCode, xerror.FormatInvalid, xerror.FormatInvalid)
			goto Label
		}

		err = in.ValidateAll()
		if err != nil {
			err = xerror.NewError(xerror.ParamValidCode, xerror.ParamValid, xerror.ParamValid)
			goto Label
		}
...
// 参考文档：https://yusank.space/posts/go-protoc-http/
